...
app.post("/upload", (req, res) => {
    ...
    uploadFile(req, res, (error) => {
        ...
        const list_object = []
        const headers = {
            "Authorization": "Client-ID 78c3310b7e9480a",
        ...
        async function makeRequest() {
            for (const item of array_img) {
                ...
                let res = await axios.post('https://api.imgur.com/3/image', buff, {headers: headers});
                list_object.push({name: item.name, url: res.data.link})
                console.log({name: item.name, url: res.data['data'].link})
            }
